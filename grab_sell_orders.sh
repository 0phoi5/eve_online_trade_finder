...
# Usage: ./grab_sell_orders.sh input.csv
...
declare -A HUB_FILE
for hub in "${HUBS[@]}"; do
	prefix="$(tr '[:upper:]' '[:lower:]' <<<"${hub%% *}")"
	HUB_FILE["$hub"]="${prefix}_sell.csv"
	echo "item_name,quantity,price" > "${HUB_FILE[$hub]}"
done

log() { [[ "${VERBOSE}" == "1" ]] && echo -e "$*"; }

# Collect up to 10 cheapest station sell orders for a type_id.
# Prints N lines: "price,quantity"
get_top_sells_in_station() {
  ...
  # First page to learn X-Pages
  curl -sD "$hdr" -o "$tmp" \
    "$ESI/markets/$region_id/orders/?order_type=sell&type_id=$type_id&datasource=$DS&page=1"
  ...
  jq --arg sid "$station_id" -r '
    map(select((.location_id|tostring)==$sid))
    | sort_by(.price)
    | .[] | "\(.price),\(.volume_remain)"
  ' "$tmp" | head -n "$want"

  # If not enough were found, continue pages
  local found
  found="$(jq --arg sid "$station_id" '
      map(select((.location_id|tostring)==$sid)) | length
    ' "$tmp")"

  if (( found < want )) && (( pages > 1 )); then
    local left=$((want - found))
    for ((p=2; p<=pages && left>0; p++)); do
      curl -s "$ESI/markets/$region_id/orders/?order_type=sell&type_id=$type_id&datasource=$DS&page=$p" \
        | jq --arg sid "$station_id" -r '
            map(select((.location_id|tostring)==$sid))
            | sort_by(.price)
            | .[] | "\(.price),\(.volume_remain)"
          ' | head -n "$left"
      left=$((left - $(curl -s "$ESI/markets/$region_id/orders/?order_type=sell&type_id=$type_id&datasource=$DS&page=$p" \
        | jq --arg sid "$station_id" 'map(select((.location_id|tostring)==$sid)) | length')))
      sleep 0.1
    done
  fi
}
...
tail -n +2 "$INPUT_CSV" | while IFS=, read -r item_id item_name _rest; do
  ...
  for hub in "${HUBS[@]}"; do
    ...
    [[ "$VERBOSE" == "1" ]] && echo -n "[${idx}/${total_items}] ${hub%% *} :: ${item_name} ... "

    lines="$(get_top_sells_in_station "${HUB_REGION_ID[$hub]}" "${HUB_STATION_ID[$hub]}" "$item_id" 10 || true)"
